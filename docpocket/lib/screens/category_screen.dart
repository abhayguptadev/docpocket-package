...
class CategoryScreen extends StatelessWidget {
  ...
  Widget _buildChip(String label, {bool isAction = false}) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      decoration: BoxDecoration(
        color: isAction ? Colors.blue[50] : Colors.white,
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: isAction ? Colors.blue[100]! : Colors.grey[300]!),
      ),
      child: Text(
        label,
        style: TextStyle(
  ...
  void _showAddDocumentBottomSheet(BuildContext context) {
    showModalBottomSheet(
      ...
      builder: (context) {
        return Center( // Center on Web for better look
          child: Container(
            constraints: const BoxConstraints(maxWidth: 500), // Constraint for Web/Large screens
            ...
            child: SingleChildScrollView( // Prevents vertical overflow
              child: Column(
                ...
                children: [
                  ...
                  // Use Wrap instead of Row to prevent horizontal overflow on narrow screens
                  Wrap(
  ...
  Widget _buildActionIcon(BuildContext context, IconData icon, String label, Color color, Function(BuildContext) onTap) {
    return InkWell(
      onTap: () => onTap(context),
      child: SizedBox(
        width: 80, // Fixed width for wrap items
        child: Column(
          children: [
            Container(
